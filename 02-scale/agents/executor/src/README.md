...
* `agents.py`: Defines the `SourcingSpecialist` and `ProcurementOfficer` using CrewAI (with Reasoning enabled).
* `tasks.py`: Defines the specific tasks (`sourcing_task`, `procurement_task`).
...
* `crew.py`: The main orchestration class that ties agents and tasks together (with Planning enabled).
