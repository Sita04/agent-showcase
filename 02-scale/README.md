...
* `--timeout 600` -- accommodates Agent Engine cold-start latency (3-5 min). Applies to all three services; the two A2A bridges previously ran at 300, which a slow Planner could exhaust.

Every in-process HTTP client must stay strictly under the Cloud Run timeout that encloses it, or the platform cuts the request first and the caller sees a bare 504 instead of a retryable failure report. Current budgets: Dashboard `AGENT_ENGINE_TIMEOUT_S = 540` under its own 600s, and the Control Room Workflow's Planner call at 240s under the bridge's 600s -- 240 leaves room for both `max_attempts` passes.
...
| Both A2A bridges start returning **504** ("maximum request timeout") on a fraction of runs, and `scale-control-room-a2a` p50 latency step-changes (e.g. ~25s → ~190s) at a precise timestamp with no deploy behind it. | One reasoning engine's serving deployment is stuck in a container restart loop: its logs show `Received SIGTERM` every few seconds and worker processes that log `Started server process` → `Shutting down` ~1ms later. Crew runs are killed mid-execution, so `:query` returns `400 FAILED_PRECONDITION ... Reasoning Engine Execution failed`, the Planner's `_query_crew_with_retry` burns all 4 attempts (3+6+12s backoff) on every delegation, and the inflated run time crosses the bridge's Cloud Run `--timeout`. | Confirm it's engine-local, not platform-wide, by comparing SIGTERM rates across all three engines (`resource.type="aiplatform.googleapis.com/ReasoningEngine" AND resource.labels.reasoning_engine_id="<id>" AND "Received SIGTERM"`) — a stuck engine shows tens per hour while its siblings show ~0. Check `protoPayload` audit logs to rule out an admin action, and the `:query` 200/400 ratio in the *caller's* engine logs to pin the cutover minute. **`update()` in place is not a durable fix** — it re-provisions the serving deployment onto the same infrastructure, and the loop came back ~3.5h later (measured: clean 01:00–03:30Z, back to incident levels by 04:00Z). What does fix it is recreating the engine so it lands on a fresh backend: `CONTROL_ROOM_STATUS_URL="${DASHBOARD_URL}/api/push_status" uv run scripts/deploy_to_agent_engine.py --crew-only --force`, then `--planning-only` **without** `--force` (the Planner bakes `crew_agent_engine_id` into its pickle, so it must be redeployed to pick up the new ID — but keeping its *own* engine ID stable means `scale-planner-a2a`'s `PLANNING_AGENT_ENGINE_ID` env var needs no change, so no Cloud Run redeploy). Recovery is immediate: `scale-control-room-a2a` p50 went 190s → 20s on the first request after cutover. The old engine's restarts stop the minute the Planner stops calling it (measured: ~48/min through 21:48:00Z, then zero), so the loop is driven by inbound requests — each one kills the container — rather than being a background crash-loop. That also means restart rate is only a valid health signal while the engine is actually receiving traffic; a quiet stuck engine looks identical to a healthy idle one. Delete the old engine once you've stopped comparing against it — and note that `--force` reuses the same `gcs_dir_name`, so while both exist they share one `pickleObjectGcsUri` and the *old* engine would load the *new* pickle if it cold-started. Back the staging dir up before deleting (`gsutil cp -r gs://${STAGING}/execution-crew gs://${STAGING}/_backup-...`) in case the delete cascades to it; it did not here (v1beta1 `DELETE ...?force=true` left all three objects intact), but the new engine boots from that exact path. Also check Personalized Service Health (`servicehealth.googleapis.com`, project-scoped — the *public* dashboard does not show these) for correlated `us-central1` events; a Cloud Run rollout that "led to task restarts" lined up with the relapse window here. |
