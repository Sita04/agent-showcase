...
| `task_type` | string | No | auto | Embedding task type: `"RETRIEVAL_QUERY"`, `"QUESTION_ANSWERING"`, or `"SEMANTIC_SIMILARITY"`. Defaults based on the dataset's embedding configuration |
...
| Dataset ID | Embedding Model | Dims | Distance Metric | BYOE | Description |
|------------|----------------|------|-----------------|------|-------------|
| `mercari3m_text_128` | gemini-embedding-001 | 128 | DOT_PRODUCT | No | Default. Reduced-dimension Gemini embeddings |
| `mercari3m_text` | gemini-embedding-001 | 768 | DOT_PRODUCT | No | Full-dimension Gemini embeddings |
| `mercari3m_text_similarity` | gemini-embedding-001 | 768 | DOT_PRODUCT | No | SEMANTIC_SIMILARITY task type for symmetric matching |
| `mercari3m_word2vec` | Gensim Word2Vec | 100 | COSINE | Yes | Word2Vec BYOE — fastest inference, word-level semantics |
| `mercari3m_multimodal` | multimodal-embedding-001 | 1408 | DOT_PRODUCT | Yes | Multimodal embeddings encoding item images |
